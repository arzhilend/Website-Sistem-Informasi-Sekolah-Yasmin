/**
 * @component BeritaTrashTable
 * @description Data table untuk menampilkan daftar berita yang dihapus
 */

<template>
  <div class="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 overflow-hidden">
    <div v-if="items.length === 0" class="p-12 text-center">
      <p class="text-gray-600 dark:text-gray-400 font-poppins font-semibold">Tempat sampah kosong</p>
      <p class="text-sm text-gray-500 dark:text-gray-500 mt-1">Tidak ada berita yang dihapus</p>
    </div>
      ...
      <table class="w-full">
        <thead class="bg-gray-50 dark:bg-gray-900 border-b border-gray-200 dark:border-gray-700">
          <tr>
            ...
            <th class="px-6 py-4 text-left text-xs font-semibold text-gray-700 dark:text-gray-300 uppercase tracking-wider font-poppins">Dihapus Pada</th>
          ...
          >
            ...
            <td class="px-6 py-4">
              <div class="flex items-center gap-3">
                <img
                  :src="getStorageUrl(berita.image, '/images/placeholder-news.jpg')"
                  :alt="berita.title"
            ...
            <td class="px-6 py-4">
              <div class="flex items-center justify-center gap-2">
                <button
                  @click="$emit('restore', berita.id)"
                  class="p-2 rounded-lg bg-green-50 dark:bg-green-900/20 text-green-600 dark:text-green-400 hover:bg-green-100 dark:hover:bg-green-900/30 transition-colors duration-200 group"
                  title="Pulihkan"
                >
                  <RotateCcw class="w-4 h-4 group-hover:scale-110 transition-transform" />
                </button>

                <button
                  @click="$emit('force-delete', berita.id)"
